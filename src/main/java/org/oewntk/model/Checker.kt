...
object Validator {
    ...
    fun <M : CoreModel> M.check(throws: Boolean = true, verbose: Boolean = false): M {
        ...
        Tracing.psInfo.println("[I] CoreModel $this checked")
        return this
    }
    ...
    fun <M : CoreModel> M.checkSynsetRelationTargets(throws: Boolean = false, verbose: Boolean = true): M {
        val instances = synsets
            ...
            .filter { (_, _, targetId) -> targetId.id[0] != 'Q' && (if (targetId.targetsSynset) synsetFinder(targetId.synsetId) == null else senseFinder(targetId.senseKey) == null) }
            .toList()
    ...
    fun <M : CoreModel> M.checkSenseRelationTargets(throws: Boolean = false, verbose: Boolean = true): M {
        val instances = senses
            ...
            .filter { (_, _, targetId) -> (if (targetId.targetsSynset) synsetFinder(targetId.synsetId) == null else senseFinder(targetId.senseKey) == null) }
            .toList()
