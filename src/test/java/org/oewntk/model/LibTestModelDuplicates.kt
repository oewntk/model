...
object LibTestModelDuplicates {
    ...
    fun testDuplicatesForKeyOEWN(model: CoreModel, ps: PrintStream) {
        testDuplicatesForKeyMono(model, { KeyF.FuncUsingPronunciation.Mono.of(Lex::lemma, { lex -> lex.partOfSpeech.toCategory() }, it) }, ps)
    }

    fun testDuplicatesForKeyPos(model: CoreModel, ps: PrintStream) {
        testDuplicatesForKeyMulti(
            model,
            { KeyF.FuncUsingPronunciation.Multi.of(Lex::lemma, { lex -> lex.partOfSpeech.toCategory() }, it) },
            ps
        )
    }
    ...
    fun testDuplicatesForKeyIC(model: CoreModel, ps: PrintStream) {
        testDuplicatesForKeyMulti(
            model,
            { KeyF.FuncUsingPronunciation.Multi.of(Lex::lCLemma, { lex -> lex.partOfSpeech.toCategory() }, it) },
            ps
        )
    }
    ...
    fun testDuplicatesForKeyPWN(model: CoreModel, ps: PrintStream) {
        testDuplicatesForKeyMulti(
            model,
            { KeyF.FuncBase.Multi.of(Lex::lCLemma, { lex -> lex.partOfSpeech.toCategory() }, it) },
            ps
        )
    }
}
